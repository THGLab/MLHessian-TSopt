...
from combust.data import parse_ani1x

# torch.autograd.set_detect_anomaly(True)
torch.set_default_tensor_type(torch.DoubleTensor)

# settings
settings_path = 'scripts/ani/wb97_newton.yml'
...
# data
train_gen, val_gen, test_gen, tr_steps, val_steps, test_steps, normalizer = parse_ani1x(settings,device[0])
...
def custom_loss(preds, batch_data, w_e=w_energy, w_f=w_force, w_fm=w_f_mag, w_fd=w_f_dir):
    ...
    if settings['checkpoint']['verbose']:
        ...
        if w_fm>0:
            print(' '*8, 'force mag loss: ', err_sq_mag_forces.detach().cpu().numpy(), '\n')
